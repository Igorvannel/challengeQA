...
test.describe('Tests de validation des champs', () => {
  ...
  test('7. Validation du champ Mot de passe - Doit afficher une erreur si le mot de passe est faible', async ({ page }) => {
    ...
    expect(isPasswordErrorDisplayed).toBe(true);
  });
...
test.describe('Tests des champs optionnels', () => {
  ...
  test('10. Validation du champ Date de naissance - Doit échouer pour un format de date invalide', async ({ page }) => {
    ...
    expect(isDobErrorDisplayed).toBe(true);
  });
  ...
  test('11. Validation du champ Numéro de téléphone - Doit afficher une erreur si le numéro est invalide', async ({ page }) => {
    ...
    const isPhoneErrorDisplayed = await userCreationProfilePage.checkForError('phone');
    expect(isPhoneErrorDisplayed).toBe(true);
  });

  test('12. Validation du champ Adresse - Doit afficher une erreur si l\'adresse est invalide', async ({ page }) => {
    ...
    expect(isAddressErrorDisplayed).toBe(true);
  });

  test('13. Validation du champ LinkedIn URL - Doit afficher une erreur pour un format d\'URL invalide', async ({ page }) => {
    await userCreationProfilePage.fillOptionalFields('', '', '', '', 'linkedin.com/johnsmith', '');
    await userCreationProfilePage.submitForm();
    
    const isLinkedinErrorDisplayed = await userCreationProfilePage.checkForError('linkedin');
    expect(isLinkedinErrorDisplayed).toBe(true);
  });
  ...
  test('14. Validation du champ GitHub URL - Doit afficher une erreur pour un format d\'URL invalide', async ({ page }) => {
    ...
    const isGithubErrorDisplayed = await userCreationProfilePage.checkForError('github');
