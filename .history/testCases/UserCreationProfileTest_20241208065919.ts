...
test.describe('Tests de soumission du formulaire', () => {
  ...
  // Test 12: Soumettre le formulaire avec des données valides
  test('Soumettre le formulaire avec des données valides', async ({ page }) => {
    await userCreationProfilePage.fillForm('John', 'Doe', 'john.doe@example.com', 'Password123!', 'Password123!');
    await userCreationProfilePage.submitForm();
    expect(await userCreationProfilePage.isFormSubmittedSuccessfully()).toBe(true);
  });
